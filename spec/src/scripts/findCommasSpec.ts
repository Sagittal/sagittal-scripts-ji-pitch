...
describe("find-commas", (): void => {
    ...
    it("can sort the resulting list by a specific term of the monzo", (): void => {
        onlyRunInCi()

        const script = `npm run find-commas -- --lower-bound 50c --upper-bound 50.31c --sort-by monzo2 --max-n2d3p9 ${OLD_MAX_N2D3P9_FOR_SHORTER_TEST_RESULTS} --max-ate ${OLD_MAX_ATE_FOR_SHORTER_TEST_RESULTS} --max-aas ${OLD_MAX_AAS_FOR_SHORTER_TEST_RESULTS}` as Io
    ...
    it("can set the format of the comma names", (): void => {
        ...
        const expected = [
            ...
            "        \t5^2*11^2-Medium-Diesis\t 1594323\t/\t1548800 \t    [\t -9    \t 13    \t -2    \t  0    \t -2    \t       \t       \t       \t⟩\t        50.152¢\t  9.912\t  9.912\t 13    \t 11     \t    3025\t/\t1 \t₂,₃\t  4     \t 32     \t231.076 ",
            "        \t19:5^2*7-Medium-Diesis\t    4864\t/\t4725    \t    [\t  8    \t -3    \t -2    \t -1    \t  0    \t  0    \t  0    \t  1    \t⟩\t        50.195¢\t -6.091\t  6.091\t  3    \t 19     \t     175\t/\t19\t₂,₃\t  4     \t 36     \t292.477 ",
            "",
        ] as Io[]
        expect(actual).toEqual(expected)
    })
