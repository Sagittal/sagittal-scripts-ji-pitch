...
const compute23FreeClassRow = (
    ...
    const { two3FreeClass, two3FreePrimeLimit, two3FreeCopfr, two3FreeSopfr, n2d3p9 } =
        two3FreeClassAnalysis

    const row = [] as unknown[] as Row<{ of: Two3FreeClassAnalysis }>
    if (
        !jiPitchScriptGroupSettings.excludedFields.includes(
            Two3FreeClassField.TWO_3_FREE_PRIME_LIMIT,
        )
    ) {
        row.push(
    ...
    if (
        !jiPitchScriptGroupSettings.excludedFields.includes(
            Two3FreeClassField.TWO_3_FREE_CLASS_NAME,
        )
    ) {
