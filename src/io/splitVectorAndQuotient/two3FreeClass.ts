...
const formatSplit23FreeClass = (two3FreeClass: Two3FreeClass): Formatted<Two3FreeClassAnalysis>[] => {
    const formatted23FreeClass = format23FreeClass(
        two3FreeClass,
    ) as Formatted as Formatted<Two3FreeClassAnalysis>

    if (ioSettings.tableFormat === TableFormat.FORUM) {
        return [formatted23FreeClass] as Formatted[] as Formatted<Two3FreeClassAnalysis>[]
    } else {
        const sign =
        ...
        return [formattedNuminator, "/", formattedDiminuator, sign] as Formatted<Two3FreeClassAnalysis>[]
    }
}

export { formatSplit23FreeClass }
