...
): [Index<Semitina>, Comma[]][] => {
    const commasBySemitinaZoneEntries = sort(
        Object.entries(commasBySemitinaZone).map(
            ([semitinaZone, commas]: [string, Comma[]]): [Index<Semitina>, Comma[]] => {
                return [parseInt(semitinaZone) as Index<Semitina>, commas]
            },
        ),
        { by: computeKeyPath(0) },
    ) as [unknown, Comma[]][] as [Index<Semitina>, Comma[]][]

    commasBySemitinaZoneEntries.forEach(([semitinaZone, commas]: [Index<Semitina>, Comma[]]): void => {
...
export { computeCommasBySemitinaZoneEntries }
